  ...
  <div class="page-container">
    <el-card shadow="hover">
      ...
      <div class="statistics-grid">
        <WmsStatisticsCard title="出库单总数" :value="stats.totalCount" :icon="Document" color="#409EFF" />
        ...
        <WmsStatisticsCard title="今日出库" :value="stats.todayCount" :icon="TrendCharts" color="#909399" />
      </div>
...
import { getOutboundStatistics } from '@/api/outbound';
import type { OutboundStatisticsDto } from '@/api/outbound';

const stats = ref<OutboundStatisticsDto>({
  totalCount: 0,
  pendingCount: 0,
  completedCount: 0,
  todayCount: 0,
...
async function loadStatistics() {
  ...
  try {
    stats.value = await getOutboundStatistics(params);
  } catch {
    // keep defaults
  }
  renderChart();
